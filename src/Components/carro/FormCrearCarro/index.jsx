...
export default function FormCrearCarro() {
    ...
    const guardarCarro = async (carroActualizado) => {
        ...
        // if (res.status === 200) router.push("/MisCarros");
        if (res.status === 200) setCodigoCarro(res?.data?.CodigoCarro);
        else console.log(res.data);
    };

    return (
        <div className="Contenedor-FormCrearCarro">
            {!codigoCarro && <form onSubmit={handleSubmit}>
            ...
            {codigoCarro && <FormSubirImagenCarro CodigoCarro={codigoCarro}/>}
        </div>
    );
}
