...
export default function CardNoDisponible({ dataCarro, esPropio }) {
    return (
        <Link href={`/Carros/${dataCarro.CodigoCarro}`}>
            <div className="contenedor-card-no-disponible">
                <Card
                    isFooterBlurred
                    radius="lg"
                    className="border-none py-2 bg-slate-400 text-white"
                >
                    <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
                        <p className="text-tiny uppercase font-bold">
                            {dataCarro.Marca} {dataCarro.Modelo}
                        </p>
                        <small className="text-default-500 text-white">
                            {dataCarro.Anio}
                        </small>
                        <h4 className="font-bold text-large">
                            Q {dataCarro.PrecioCliente} por dia
                        </h4>
                    </CardHeader>
